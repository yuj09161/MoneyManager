                ...
                if month_data.size:
                    ...
                    for t in range(2):
                        for s in self.__sources:
                            type_sum = month_data[
                                (month_data['type'] == t) & (month_data['src'] == s)
                            ]['val'].sum()
                    ...
                    if c3 != c5 or c4 != c6 or c7 != c8:
                        ...
                        if not any((c3, c4, c7)):
                            self.appendRow(self.__qitem_vec(
                                (f'최초', c2, cash, 0, 0, 0, 0)).tolist())
                        else:
                            self.appendRow(self.__qitem_vec(
                                (f'{y}-{m}', c2, cash, c3-c4, c3, c4, c7)).tolist())
        ...
        if m_c in self.__month_list:
            ...
            if c3 != c5 or c4 != c6 or c7 != c8:
                ...
                items = self.__qitem_vec(
                    (f'{m_c[0]}-{m_c[1]}', c2, cash, c3-c4, c3, c4, c7)).tolist()
                if insert:
...
class Login(QMainWindow, Ui_Login):
    ...
    def __set_file(self):
        self.__file = None
        self.__file, _ = QFileDialog.getOpenFileName(
    ...
    def __init__(self, file_name):
        ...
        try:
            ...
            self.__opensource_win = Info(
                self, 'Open Source License', 'Open Source Notice file (Notice) does not exist')
        else:
        ...
        if text and text != '-': # if month -> set data
            ...
            if raw_data:
                ...
                for wids, type_, changer in zip(
                            self.__data_labels, self.__stat.data_name[2:], data_changer
                        ):
